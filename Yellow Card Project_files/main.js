...
var bgPrimary = '#4a89dc',
 ...
 $('#rregion').on('change', function() {
	  ...
	  $.ajax({
			type: "POST",
			url: "https://yellowcardnigeria.com/users/portsBYregion",
			data: {'regionid': regionval},
			success: function(data) {
				var result = JSON.parse(data);
				$('.registerport').html(result.portoptions);
			}
		});
  });

	jQuery(document).ready(function(){
		$("#rport").chosen({width: "95%", placeholder_text: "Select Port"}); 
		$("#rport option[value='spcl']").remove(); 
		$('#rport').trigger("chosen:updated");
	});
	
	$("#add_portregion").submit(function(event){
		event.preventDefault();
		var region = $('#region').val();
		var portarr = [];
		$( ".ports_cs ul li span" ).each(function( index ) {
			var port = $( this ).html();
			 portarr.push({ port });
		});		
		 $.ajax({
			type: "POST",
			url: "http://yellowcardnigeria.com/users/add_portregion",
			data: {'region': region,'ports': portarr},
			success: function(data) {
			  var result = JSON.parse(data);
			  if(result.status==1){
				  var alertmsg = '<div class="alert alert-success pastel alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">Ã—</button><i class="fa fa-trophy pr10"></i><strong>Well Done!</strong>'+ result.msg+'</div>';
			  }else{
				  var alertmsg = '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">Ã—</button><i class="fa fa-remove pr10"></i><strong>Oh snap!</strong> '+ result.msg+'</div>';
			  }
				$('#rport').val('').trigger('chosen:updated');
				document.getElementById("add_portregion").reset(); 
				window.location.href = 'http://yellowcardnigeria.com/users/addnewPort';
			}
		});
	});
	$(".admin_region").change(function(){
	   var regionval = $(this).val();
	   if(regionval==37){
			$('#rport').append('<option value="spcl">Special Port</option>');
			$('#rport').trigger("chosen:updated");
	   }else{
		   $("#rport option[value='spcl']").remove(); 
		   $('#rport').trigger("chosen:updated");
	...
	$(".statefilter").change(function(){
		...
		$.ajax({
			type: "POST",
			url: "http://yellowcardnigeria.com/users/govtbyState",
			data: {'stateid': stateid},
			success: function(data) {
				var result = JSON.parse(data);
				$('.govtfilter').html(result);
			}
		});
	});